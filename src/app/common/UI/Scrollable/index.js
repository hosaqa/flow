...
const Viewport = styled.div`
  height: ${({ viewportHeight }) =>
    viewportHeight ? `${viewportHeight}px` : 'auto'};
  overflow: hidden;
  position: relative;
`;
...
export default class Scrollable extends Component {
  ...
  getViewportHeight = () =>
    findDOMNode(this.viewport).parentElement.clientHeight;
  ...
  scrollTo = to => {
    ...
    const toExtremeLimitRewindCondition =
      delta > 0
        ? contentPosition - delta < extremeLimit
        : contentPosition - delta > extremeLimit;

    if (contentPosition !== extremeLimit) {
      const to = toExtremeLimitRewindCondition
        ? extremeLimit
        : this.state.contentPosition - delta;
        ...
        >
          {interpolatedStyle => (
            <ContentWrapper contentPosition={interpolatedStyle.top}>
              <Content ref={this.getContentRef} contentHeight={contentHeight}>
                {children}
              </Content>
            </ContentWrapper>
          )}
